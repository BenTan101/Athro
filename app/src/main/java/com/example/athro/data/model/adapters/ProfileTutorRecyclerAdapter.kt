...
class ProfileTutorRecyclerAdapter(
    val fragment: ProfileFragment,
    val database: DatabaseReference,
    val progUser: User
) :
    RecyclerView.Adapter<ProfileTutorRecyclerAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.user.text = students[position].name
        holder.year.text =
            String.format(fragment.getString(R.string.year_what), students[position].year)
        ...
        holder.submitButton.setOnClickListener {
            students[position].tutee!!.reviews.add(
                Review(
            ...
            database.child("users").child(students[position].email!!.substring(0, 8))
                .setValue(students[position])

            Toast.makeText(
                fragment.context,
                String.format(fragment.getString(R.string.reviewed_who), students[position].name),
                Toast.LENGTH_SHORT
            )
                .show()
